...
import { ScanLine, ClipboardList, LogOut, Menu, X, ChevronRight, Settings } from 'lucide-react';
import { useAuth } from '../features/auth/AuthContext';
import ThemeToggle from '../components/ui/ThemeToggle';
import logo from '../assets/jkuat-logo.png';

const BRAND = '#1A5276';
const BRAND_DARK = '#154360';
const GOLD = '#F5A623';

const navItems = [
  { to: '/invigilator/scanner', label: 'Scanner',      icon: ScanLine },
  { to: '/invigilator/history', label: 'Scan History', icon: ClipboardList },
  { to: '/invigilator/settings',label: 'Settings',     icon: Settings },
];

const pageTitles = {
  '/invigilator/scanner':  'QR Scanner',
  '/invigilator/history':  'Scan History',
  '/invigilator/settings': 'Settings',
};

function Sidebar({ initials, fullName, staffNumber, onLogout, onNavClick }) {
  return (
    <aside style={{ background: BRAND, width: 256 }} className="flex flex-col h-full flex-shrink-0">
      <div className="flex items-center gap-3 px-5 py-5"
        ...
        <div>
          ...
          <p className="text-xs leading-tight" style={{ color: 'rgba(255,255,255,0.5)' }}>Invigilator Portal</p>
        </div>
      </div>

      <nav className="flex-1 px-3 py-5 space-y-1 overflow-y-auto">
        <p className="px-3 mb-3 text-xs font-semibold uppercase tracking-widest"
          style={{ color: 'rgba(255,255,255,0.35)' }}>Navigation</p>
      ...
      <div className="px-4 py-4" style={{ borderTop: '1px solid rgba(255,255,255,0.1)' }}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs truncate" style={{ color: 'rgba(255,255,255,0.5)' }}>{staffNumber}</p>
          </div>
        </div>
...
export default function InvigilatorLayout() {
  ...
  const fullName = user?.fullName ?? 'Invigilator';
  const staffNumber = user?.staffNumber ?? '';
  const initials = fullName ? fullName[0].toUpperCase() : 'I';
  const pageTitle = pageTitles[location.pathname] ?? 'Portal';

  const handleLogout = () => { logout(); navigate('/invigilator/login'); };

  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--surface)' }}>
      <div className="hidden lg:flex">
        <Sidebar initials={initials} fullName={fullName} staffNumber={staffNumber} onLogout={handleLogout} />
      </div>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 flex lg:hidden">
          ...
          <div className="relative z-50">
            <Sidebar initials={initials} fullName={fullName} staffNumber={staffNumber}
              onLogout={handleLogout} onNavClick={() => setSidebarOpen(false)} />
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-16 flex items-center px-4 lg:px-6 gap-4 flex-shrink-0"
          ...
          <div className="hidden sm:flex items-center gap-2 text-sm">
            <span style={{ color: 'var(--text-muted)' }}>Invigilator Portal</span>
          ...
          <div className="flex items-center gap-2.5 pl-3" style={{ borderLeft: '1px solid var(--border)' }}>
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{staffNumber}</p>
            </div>
          </div>
        </header>
